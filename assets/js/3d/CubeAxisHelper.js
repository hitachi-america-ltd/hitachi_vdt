CubeAxisHelper = function( sourceCamera, font, htmlDOM ) {

    if( sourceCamera === undefined || font === undefined || htmlDOM === undefined ){

        console.warn( "%cCubeAxisHelper( sourceCamera, font, htmlDOM ) : Constructor expects the following arguments,\n\"sourceCamera\" : The camera using which the CubeAxisHelper is to be rendered,\n\"font\" : The font family that should be used for rendering the text,\n\"htmlDOM\" : The HTML DOM element to which the CubeAxisHelper should be attached", "font-style: italic; color: yellow; background-color: blue; padding: 2px;" );
        return false;
    ...
    htmlDOM.appendChild( this.container );
    //this.container.id = "axes";
    this.container.setAttribute( "id", "editor-cube-axis-helper" );
    this.styleContainer();
    this.visible = true;
    ...
    //var cubeMat = new THREE.MeshPhongMaterial( { color: 0xededed, shininess: 10 } );
    var cubeMat = new THREE.MeshPhongMaterial( { color: 0xffffff, shininess: 10 } );
    ...
    //fontLoader.load( './js/helvetiker_regular.typeface.json', function( font ) {
        ...
        geos[ 2 ].rotateX( Math.PI / 2 );
        geos[ 2 ].rotateZ( Math.PI );//
        //geos[ 2 ].translate( -2, -dist, -2 );
        geos[ 2 ].translate( 4, dist, -2 );//

        geos[ 3 ].rotateX( Math.PI / 2 );
        //geos[ 3 ].rotateZ( Math.PI );
        //geos[ 3 ].translate( 4, dist, -2 );
        geos[ 3 ].translate( -2, -dist, -2 );

        //geos[ 4 ].translate( -2, -2, dist );
        geos[ 4 ].rotateY( Math.PI );//
        geos[ 4 ].translate( 4, -2, -dist );//

        //geos[ 5 ].rotateY( Math.PI );
        //geos[ 5 ].translate( 4, -2, -dist );
        geos[ 5 ].translate( -2, -2, dist );//
...
CubeAxisHelper.prototype.toggleVisibility = function() {
...
CubeAxisHelper.prototype.update = function() {

    var camPos = this.sourceCamera.getWorldDirection( new THREE.Vector3() );
...
CubeAxisHelper.prototype.styleContainer = function() {
...
CubeAxisHelper.prototype.updateCamera = function( camera ) {

    if( camera === undefined ){

        console.warn( "%cCubeAxisHelper.updateCamera( camera ) : Camera should be provided", "font-style: italic; color: yellow; background-color: blue; padding: 2px;" );
