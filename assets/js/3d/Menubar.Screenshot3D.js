...
/**
 * Menubar.Screenshot3D( editor ) : Constructor for taking the snapshot of 3D view
 * @constructor
 * @param {any} editor - The instance of the Editor which manages the overall objects and signals
 * @returns {Object<UI.Panel>}
 * @author Bharath
 * @example <caption>Example usage of Menubar.Screenshot3D</caption>
 * var menubarScreenshot3D = new Menubar.Screenshot3D( editor );
 */

Menubar.Screenshot3D = function(editor) {
    ...
    title.setId('screenshot_3d');
    title.setStyle('display', ['block']);

    title.onClick(function() {
        var date = new Date;
        var classListName = document.getElementById("screenshot_3d");
    
        if (editor.activeProject.user_id != undefined) {
            document.getElementById("screenshot_3d").className += " selectMenubar";
            ...
            function dataURItoBlob(dataURI, type) {
                ...
                if (editor.selectedView == true) {
                    ...
                    editor.filenametwod = "3D_View_"  + Date.now() + "(" + cameraX + ',' + cameraY + ',' + cameraZ + 'Deg' + cameraTiltX + ',' + cameraTiltY + ',' + cameraTiltZ + ')' + "editorCamera";
